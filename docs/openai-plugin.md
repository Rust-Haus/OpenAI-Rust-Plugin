...
**For plugin developers:** See [Developer Hooks](developer-hooks.md) for complete hook documentation and examples.
...
VIP Tiers let you give selected players a different AI model and higher token limits (daily limit, max output tokens, cooldown, reasoning effort, web search) without editing Steam ID lists. Each tier is permission-based: the permission is always `openai.vip.<tier_name>` (e.g. `openai.vip.vip_elite`). The plugin applies that tier's model and per-player limits when the player uses the AI. VIP does not bypass rate limits; `openai.unlimited` is unchanged.

See [VIP Tiers](vip-tiers.md) for setup, config, and commands.
